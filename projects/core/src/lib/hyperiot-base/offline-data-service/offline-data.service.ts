...
export class OfflineDataService extends BaseDataService {
  ...
  scanAndSaveHProject(dataChannel: DataChannel, deviceId, alarmState): Observable<PacketDataChunk[]> {
    ...
    return this.hprojectsService.scanHProject1(this.hProjectId, dataChannel.controller.channelLowerBound, this.dashboardRowKeyUpperBound, this.DEFAULT_CHUNK_LENGTH, packetIds.toString(), deviceId, alarmState).pipe(
      map(res => {
